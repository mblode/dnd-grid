...
export class ReactGridLayout extends React.Component<Props, State> {
  ...
  handleDndRect = (
    ...
    const finalDroppingItem = { ...droppingItem, ...onDragOverResult };
    const { layout } = this.state;
    const gridEl = document.getElementById("react-grid-layout");
    const gridRect: any = gridEl?.getBoundingClientRect();
    // Calculate the mouse position relative to the grid
