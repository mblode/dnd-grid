...
type EventCallback = (
  ...
  oldItem: LayoutItem,      // Item state before the operation
  newItem: LayoutItem,      // Item state after the operation
  placeholder: LayoutItem,  // Placeholder position (during drag/resize)
  event: MouseEvent,        // Original mouse/touch event
  element: HTMLElement      // DOM element being manipulated
) => void;
```
