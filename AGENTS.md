...
- **Never add a `paths` mapping for `@dnd-grid/core`** in `packages/dnd-grid-react/tsconfig*.json`. TypeScript bakes the resolved path into the emitted `.d.ts`, so the published package ships `from '../../dnd-grid-core/dist/index.d.ts'`, which does not exist in the tarball and breaks consumers' types. Verify with: `npm run build && grep -r "dnd-grid-core/dist" packages/dnd-grid-react/dist/` (must print nothing).
