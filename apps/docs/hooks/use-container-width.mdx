...
`DndGrid` needs a fixed width. `useContainerWidth` watches a wrapper element
with `ResizeObserver` and gives you a width that stays in sync as the container
resizes.
...
<Note>
  Prefer `AutoWidthDndGrid` if you want the same behavior without wiring the
  hook yourself.
</Note>
