...
describe("utils", () => {
	...
	describe("cloneLayoutItem", () => {
		...
		it("converts moved and static to booleans", () => {
			...
			const cloned = cloneLayoutItem(item as any);
		...
		it("converts truthy values to true for boolean fields", () => {
			const item = { ...createLayoutItem(), moved: 1, static: "yes" };
			const cloned = cloneLayoutItem(item as any);
	...
	describe("setTransform", () => {
		...
		it("handles undefined deg", () => {
			const pos = { left: 0, top: 0, width: 100, height: 100 };
			const style = setTransform(pos as any);
	...
	// --- COMPACT TYPE HELPER ---
	describe("compactType helper", () => {
		it("returns compactType when verticalCompact is true", () => {
			expect(
				compactType({ verticalCompact: true, compactType: "horizontal" }),
			).toBe("horizontal");
		});

		it("returns null when verticalCompact is false", () => {
			expect(
				compactType({ verticalCompact: false, compactType: "vertical" }),
			).toBeNull();
		});

		it("handles null props", () => {
			expect(compactType(null)).toBeUndefined();
		});

		it("handles undefined props", () => {
			expect(compactType(undefined)).toBeUndefined();
		});

		it("returns vertical when verticalCompact is true and compactType is vertical", () => {
			expect(
				compactType({ verticalCompact: true, compactType: "vertical" }),
			).toBe("vertical");
		});
	});
