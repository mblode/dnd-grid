...
| `wrapCompactor` | Wraps items in row-major order (from `extras`). |
| `wrapOverlapCompactor` | Wrap mode with overlap allowed. |
...
- `getCompactor` for compatibility with the old `compactType` API
...
const myCompactor: Compactor = {
  ...
  onMove(layout, item, x, y) {
    return layout.map((entry) =>
      entry.i === item.i ? { ...entry, x, y, moved: true } : entry,
    );
  },
};
```
