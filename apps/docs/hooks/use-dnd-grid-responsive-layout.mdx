...
layouts, and returns ready-to-spread props for `DndGrid`.
...
| `margin` | `ResponsiveSpacing` | Gap per breakpoint (number or spacing object). |
| `containerPadding` | `ResponsiveSpacing | null` | Padding per breakpoint (or `null` to use margin). |
...
| `onWidthChange` | `(width, margin, cols, padding) => void` | Fires on width or breakpoint config changes. |
...
| `margin` | Resolved margin for the current breakpoint. |
| `containerPadding` | Resolved container padding for the current breakpoint. |
| `gridProps` | `{ layout, cols, margin, containerPadding }` to spread onto `DndGrid`. |
