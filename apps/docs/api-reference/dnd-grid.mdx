...
<ResponseField name="children" type="ReactNode" required>
  Grid items. Each child must have a unique `key` matching a layout item's `i`
  property.
</ResponseField>
...
<ResponseField
  ...
  default="null"
>
  Padding inside the container: `[top, right, bottom, left]`. If null, uses
  margin values.
</ResponseField>
...
<ResponseField
  ...
  default='"vertical"'
>
  Direction to compact items after drag/resize. - `"vertical"` - Compact upward
  - `"horizontal"` - Compact leftward - `null` - No compaction
</ResponseField>
...
<ResponseField name="dragTouchDelayDuration" type="number" default="250">
  Milliseconds to wait before drag starts on touch devices. Helps distinguish
  scroll from drag.
</ResponseField>
...
<ResponseField name="transformScale" type="number" default="1">
  Scale factor for CSS transforms. Use when the grid is inside a scaled
  container.
</ResponseField>
